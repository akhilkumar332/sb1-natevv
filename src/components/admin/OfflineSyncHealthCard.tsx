    ...
    <div className="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100">{t('admin.offlineSyncHealth')}</h3>
        </div>
        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-semibold ${isOnline ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300'}`}>
          ...
          {isOnline ? t('common.active') : t('network.offline')}
        </span>
      </div>

      <div className="grid grid-cols-2 gap-2 text-sm text-gray-700 dark:text-slate-300">
        <p className="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 dark:border-slate-700"><span>{t('admin.queued')}</span><span className="font-semibold">{telemetry.pendingCount}</span></p>
        <p className="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 dark:border-slate-700"><span>{t('admin.failedItems')}</span><span className="font-semibold text-rose-700 dark:text-rose-300">{deadLetters.length}</span></p>
        <p className="col-span-2 flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 dark:border-slate-700"><span>{t('admin.syncSuccess')}</span><span className="font-semibold text-emerald-700 dark:text-emerald-300">{successRate.toFixed(1)}%</span></p>
      </div>
      ...
      >
        {t('admin.openFullOfflineSyncHealth')}
      </a>
    </div>
  );
}
